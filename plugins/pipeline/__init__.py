...
PIPELINE_START_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "acceptance": {
                "type": "object",
                "description": "验收标准（多维度）",
                "properties": {
                    "max_failed": {
                        "type": "integer",
                        "description": "允许失败的最大数量（默认 0）",
                        "default": 0,
                    },
                    "min_passed": {
                        "type": "integer",
                        "description": "要求通过的最小数量",
                    },
                    "must_pass": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "必须一次过的测试 ID 列表（如 ['T1', 'T4']），失败即 block",
                    },
                    "must_not_regress": {
                        "type": "boolean",
                        "description": "修复后不能引入新失败（默认 true）",
                        "default": True,
                    },
                    "max_duration_sec": {
                        "type": "integer",
...
def handle_pipeline_start(args: dict, **kw) -> str:
    ...
    acceptance: dict = args.get("acceptance") or {"max_failed": 0}
    ...
    max_failed: int = acceptance.get("max_failed", 0)
    min_passed: int | None = acceptance.get("min_passed")
    ...
    while retry_count <= max_retries:
        ...
        failed = evidence.get("failed", 999)
        ...
        # Gate: min_passed (only if set)
        if verdict == "pass" and min_passed is not None:
            if passed < min_passed:
                verdict = None  # downgrade

        # Gate: must_pass（关键测试必须一次过）
        must_pass_list = acceptance.get("must_pass", [])
        if verdict == "pass" and must_pass_list:
            tests = evidence.get("tests", [])
            for tid in must_pass_list:
                test = next((t for t in tests if t.get("id") == tid or t.get("name") == tid), None)
                if test is None:
                    logger.info("must_pass test not found in evidence: %s", tid)
                    verdict = None
                    break
                if test.get("status") != "pass":
                    logger.info("must_pass failed: %s status=%s", tid, test.get("status"))
                    verdict = None
                    break

        # Gate: must_not_regress（修复后不能引入新失败）
        # 在证据入栈前比较，避免与当前轮自比
        if verdict == "pass" and acceptance.get("must_not_regress", True):
            if evidence_history:
                prev_failed = evidence_history[-1].get("failed", 0)
                if failed > prev_failed:
                    logger.info("regression detected: %d -> %d", prev_failed, failed)
                    verdict = None
